        ...
        with patch.object(repository, 'create_company', return_value=MagicMock(ticker="AAPL", name="Apple Inc.")):
            with patch.object(repository, 'create_document', return_value=MagicMock(
                ...
                with patch.object(repository, 'create_content_chunk', return_value=MagicMock()):
                    with patch.object(repository, 'create_text_chunk', return_value=MagicMock()):
                        # Create processor
                        ...
                        with patch('farsight2.document_processing.document_processor.EdgarClient') as mock_edgar:
                            ...
                            with open(temp_file, 'w') as f:
                                f.write('<html><body><h1>ITEM 1. BUSINESS</h1><p>Test business description.</p></body></html>')
                            ...
                            with patch.object(processor, '_parse_document', return_value=MagicMock(
                                ...
                                document = processor.process_document(temp_file, "AAPL", 2023, None, "10-K")
...
def test_query_workflow(mock_get, repository, embedding_service, temp_file):
    ...
    with patch.object(QueryAnalyzer, 'analyze_query', return_value=MagicMock(
        companies=["AAPL"],
        years=[2023],
        quarters=[],
        topics=["revenue"]
        ...
        with patch.object(ContentRetriever, 'retrieve_content', return_value=mock_chunks):
            mock_response = MagicMock(
            ...
            with patch.object(ResponseGenerator, 'generate_response', return_value=mock_response):
                with patch.object(repository, 'get_document', return_value=MagicMock(
                    document_id="DOC123",
                    ticker="AAPL",
                    year=2023,
                    quarter=None,
                    ...
                    retriever = ContentRetriever(repository=repository, embedding_service=embedding_service)
                    ...
                    # Verify
                    assert response.answer == "Apple's revenue in 2023 was $100 billion."
