...
def test_create_company(repository):
    ...
    with patch.object(repository._repos['company'], 'create_company', return_value=mock_company):
        with patch.object(repository._repos['company'], 'to_model', return_value=MagicMock(ticker="AAPL", name="Apple Inc.")):
...
def test_get_company(repository):
    ...
    # Fix: Use patch to mock the methods
    with patch.object(repository._repos['company'], 'get_company', return_value=mock_company):
        with patch.object(repository._repos['company'], 'to_model', return_value=MagicMock(ticker="AAPL", name="Apple Inc.")):
            # Call the method under test
...
def test_create_document(repository, sample_document):
    ...
    # Fix: Use patch to mock the methods
    with patch.object(repository._repos['document'], 'create_document', return_value=mock_document):
        with patch.object(repository._repos['document'], 'to_model', return_value=sample_document):
            # Call the method under test
...
def test_delete_document(repository):
    ...
    with patch.object(repository._repos['document'], 'get_document', return_value=mock_document):
        with patch.object(repository._repos['chunk'], 'get_chunks_by_document', return_value=[]):
            with patch.object(repository._repos['text_chunk'], 'get_text_chunks_by_document', return_value=[]):
                with patch.object(repository._repos['table'], 'get_tables_by_document', return_value=[]):
                    with patch.object(repository._repos['chart'], 'get_charts_by_document', return_value=[]):
                        with patch.object(repository._repos['document'].db, 'delete'):
                            with patch.object(repository._repos['document'].db, 'commit'):
                                # Call the method under test
