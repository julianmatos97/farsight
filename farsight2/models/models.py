...
class QueryAnalysis(BaseModel):
    ...
    requires_numerical_data: bool = Field(False, description="Whether the query requires numerical data")
    data_types: List[str] = Field(default_factory=list, description="Types of data needed (e.g., text, table, chart, xbrl_fact)")
...
class Citation(BaseModel):
    ...
    content_type: str = Field(..., description="Type of content (text, table, chart, xbrl_fact)")
    fact_id: Optional[str] = Field(None, description="ID of the XBRL fact if applicable")
...
class QueryResponse(BaseModel):
    ...
    facts_used: List[str] = Field(default_factory=list, description="IDs of XBRL facts used")
...
class Fact(BaseModel):
    ...
    """
    fact_id: str = Field(..., description="Unique fact ID (usually taxonomy + concept name)")
    label: str = Field("No label available", description="Human-readable label for the fact")
    description: str = Field("No description available", description="Detailed description of the fact")
    taxonomy: str = Field("us-gaap", description="Taxonomy namespace (us-gaap, dei, etc.)")
    fact_type: str = Field("monetary", description="Type of fact (monetary, string, date, etc.)")
    period_type: Optional[str] = Field(None, description="Period type (instant or duration)")
    embedding: Optional[List[float]] = Field(None, description="Vector embedding of the fact's label and description")
...
class FactValue(BaseModel):
    ...
    # Document reference
    document_id: Optional[str] = Field(None, description="ID of the filing document containing this value")
    filing_type: Optional[str] = Field(None, description="Filing type (10-K, 10-Q, etc.)")
    accession_number: Optional[str] = Field(None, description="SEC accession number")
    
    # Time context
    start_date: Optional[str] = Field(None, description="Start date of period (for duration facts)")
    end_date: Optional[str] = Field(None, description="End date of period (for all facts)")
    fiscal_year: Optional[int] = Field(None, description="Fiscal year")
    fiscal_period: Optional[str] = Field(None, description="Fiscal period (Q1, Q2, Q3, Q4, FY)")
    
    # Additional context
    unit: Optional[str] = Field("USD", description="Unit of measurement (USD, shares, etc.)")
    ...
    quarter_over_quarter_change: Optional[float] = Field(None, description="QoQ percentage change")
...
class XBRLContext(BaseModel):
    ...
    instant: Optional[datetime] = Field(None, description="Point-in-time date for instant facts")
    scenario: Optional[str] = Field(None, description="Scenario information (actual, forecast, etc.)")
    segment: Optional[Dict[str, Any]] = Field(None, description="Segment information (business segment, etc.)")
...
class XBRLUnit(BaseModel):
    ...
    divide_numerator: Optional[str] = Field(None, description="Numerator for division units")
    divide_denominator: Optional[str] = Field(None, description="Denominator for division units")
...
class XBRLFactValue(BaseModel):
    ...
    document_id: str = Field(..., description="ID of the document containing this value")
    unit: Optional[str] = Field(None, description="Unit of measurement")
