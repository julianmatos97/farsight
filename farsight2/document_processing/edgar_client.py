...
class EdgarClient:
    ...
    XBRL_COMPANYCONCEPT_URL = "https://data.sec.gov/api/xbrl/companyconcept/CIK{cik}/us-gaap/{concept}.json"
    ...
    def find_filing_url(self, ticker: str, year: int, quarter: Optional[int], filing_type: str) -> Dict[str, Any]:
        ...
        return {"url": url, "xbrl_url": xbrl_url, "accession_number": accession_number, "filing_date": date_obj}
    ...
    def get_xbrl_facts_url(self, ticker: str) -> requests.Response:
        ...
        try:
            ...
            debug_file = os.path.join(self.download_dir, f"{ticker}_xbrl_facts_debug.json")
            with open(debug_file, 'w') as f:
    ...
    def download_xbrl_facts(self, ticker: str) -> Tuple[List[Fact], List[FactValue]]:
        ...
        try:
            ...
            for taxonomy in self.SUPPORTED_TAXONOMIES:
                ...
                logger.info(f"Processing {len(taxonomy_facts)} facts from {taxonomy} taxonomy")
                ...
                for concept_name, concept_data in taxonomy_facts.items():
                    ...
                    fact_type = "monetary" if "USD" in units else "shares" if "shares" in units else "other"
                    ...
                    for unit_type, values in units.items():
                        for value in values:
                            try:
                                ...
                                # Map fiscal period
                                fiscal_period = "FY" if fp == "FY" else f"Q{fp[-1]}" if fp.startswith("Q") else fp
                                
                                # Create a document ID based on available information
                                document_id = f"{ticker}_{fy}_{fiscal_period}_{form.replace('-', '')}"
                                ...
                                logger.warning(f"Error processing fact value for {fact_id}: {str(e)}")
            
            logger.info(f"Extracted {len(facts)} facts and {len(fact_values)} fact values for {ticker}")
    ...
    def save_xbrl_facts(self, ticker: str, facts: List[Fact]) -> None:
        ...
        logger.info(f"Facts for {ticker}: saved {saved_count}, skipped {skipped_count}, errors {error_count}")
    ...
    def save_xbrl_fact_values(self, ticker: str, fact_values: List[FactValue]) -> None:
        ...
        for fact_value in fact_values:
            try:
                ...
                logger.error(f"Error saving fact value for {fact_value.fact_id}: {str(e)}")
                error_count += 1
        
        logger.info(f"Fact values for {ticker}: saved {saved_count}, skipped {skipped_count}, errors {error_count}")
    ...
    def get_financial_metrics(self, ticker: str, concept: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting financial metrics for {ticker}/{concept}: {str(e)}")
            raise
        ...
        try:
            ...
            for fact_id, values in fact_value_groups.items():
                fact_value_groups[fact_id] = sorted(
                    values, 
                    key=lambda fv: (fv.fiscal_year, self._fiscal_period_to_number(fv.fiscal_period))
                )
            ...
            for metric in key_metrics:
                if metric in fact_value_groups and len(fact_value_groups[metric]) >= 2:
                    ...
                    for i in range(1, len(values)):
                        if values[i-1].value != 0 and values[i].fiscal_period == values[i-1].fiscal_period:
                            yoy_change = (values[i].value - values[i-1].value) / abs(values[i-1].value) * 100
            ...
            for rev in revenue_values:
                for ni in net_income_values:
                    if rev.fiscal_year == ni.fiscal_year and rev.fiscal_period == ni.fiscal_period and rev.value != 0:
            ...
            logger.info(f"Calculated {len(derived_metrics)} derived metrics for {ticker}")
    ...
    def find_xbrl_fact_locations(self, ticker, document_id):
        ...
        # Find all iXBRL tags
        ixbrl_elements = soup.find_all(attrs={"name": lambda x: x and x.startswith("ix:")})
        fact_locations = {}
